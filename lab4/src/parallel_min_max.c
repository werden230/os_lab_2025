#include <ctype.h>
#include <limits.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <signal.h>

#include <sys/time.h>
#include <sys/types.h>
#include <sys/wait.h>

#include <getopt.h>

#include "find_min_max.h"
#include <errno.h>
#include "utils.h"

volatile sig_atomic_t timeout_occurred = 0;

void timeout_handler(int sig) {
    timeout_occurred = 1;
}

int main(int argc, char **argv) {
    int seed = -1;
    int array_size = -1;
    int pnum = -1;
    int timeout = 0;
    bool with_files = false;

    while (true) {
        int current_optind = optind ? optind : 1;

        static struct option options[] = {{"seed", required_argument, 0, 0},
                                          {"array_size", required_argument, 0, 0},
                                          {"pnum", required_argument, 0, 0},
                                          {"timeout", required_argument, 0, 0},
                                          {"by_files", no_argument, 0, 'f'},
                                          {0, 0, 0, 0}};

        int option_index = 0;
        int c = getopt_long(argc, argv, "f", options, &option_index);

        if (c == -1) break;

        switch (c) {
            case 0:
                switch (option_index) {
                    case 0:
                        seed = atoi(optarg);
                        if (seed <= 0) {
                            printf("Seed must be a positive number\n");
                            return 1;
                        }
                        break;
                    case 1:
                        array_size = atoi(optarg);
                        if (array_size <= 0) {
                            printf("Array size must be a positive number\n");
                            return 1;
                        }
                        break;
                    case 2:
                        pnum = atoi(optarg);
                        if (pnum <= 0) {
                            printf("Process number must be a positive number\n");
                            return 1;
                        }
                        break;
                    case 3:
                        timeout = atoi(optarg);
                        if (timeout <= 0) {
                            printf("Timeout must be a positive number\n");
                            return 1;
                        }
                        break;
                    case 4:
                        with_files = true;
                        break;

                    default:
                        printf("Index %d is out of options\n", option_index);
                }
                break;
            case 'f':
                with_files = true;
                break;

            case '?':
                break;

            default:
                printf("getopt returned character code 0%o?\n", c);
        }
    }

    if (optind < argc) {
        printf("Has at least one no option argument\n");
        return 1;
    }

    if (seed == -1 || array_size == -1 || pnum == -1) {
        printf("Usage: %s --seed \"num\" --array_size \"num\" --pnum \"num\" [--timeout \"num\"] [--by_files]\n",
               argv[0]);
        return 1;
    }

    // Устанавливаем таймаут
    pid_t timeout_pid = 0;
    if (timeout > 0) {
        printf("Timeout set to %d seconds\n", timeout);
        
        timeout_pid = fork();
        if (timeout_pid == 0) {
            sleep(timeout);
            kill(getppid(), SIGUSR1);
            exit(0);
        } else if (timeout_pid < 0) {
            printf("Failed to create timeout process\n");
            return 1;
        }
        
        struct sigaction sa;
        sa.sa_handler = timeout_handler;
        sigemptyset(&sa.sa_mask);
        sa.sa_flags = 0;
        sigaction(SIGUSR1, &sa, NULL);
    }

    int *array = malloc(sizeof(int) * array_size);
    GenerateArray(array, array_size, seed);
    int active_child_processes = 0;

    // Создаем пайпы или файлы 
    int *pipefds = NULL;
    char **filenames = NULL;
    
    if (!with_files) {
        pipefds = malloc(2 * pnum * sizeof(int));
        for (int i = 0; i < pnum; i++) {
            if (pipe(pipefds + 2*i) == -1) {
                perror("pipe failed");
                return 1;
            }
        }
    } else {
        filenames = malloc(pnum * sizeof(char*));
        for (int i = 0; i < pnum; i++) {
            filenames[i] = malloc(20 * sizeof(char));
            snprintf(filenames[i], 20, "result_%d.txt", i);
        }
    }

    struct timeval start_time;
    gettimeofday(&start_time, NULL);

    // Сохраняем PID дочерних процессов
    pid_t *child_pids = malloc(pnum * sizeof(pid_t));

    for (int i = 0; i < pnum; i++) {
        pid_t child_pid = fork();
        child_pids[i] = child_pid;
        
        if (child_pid >= 0) {
            // successful fork
            active_child_processes += 1;
            if (child_pid == 0) {
                // child process
                
                int chunk_size = array_size / pnum;
                int start = i * chunk_size;
                int end = (i == pnum - 1) ? array_size : start + chunk_size;
                
                int local_min = INT_MAX;
                int local_max = INT_MIN;
                
                for (int j = start; j < end; j++) {
                    if (array[j] < local_min) local_min = array[j];
                    if (array[j] > local_max) local_max = array[j];
                }
                
                if (with_files) {
                    FILE *file = fopen(filenames[i], "w");
                    if (file) {
                        fprintf(file, "%d %d", local_min, local_max);
                        fclose(file);
                    }
                } else {
                    close(pipefds[2*i]);
                    FILE *stream = fdopen(pipefds[2*i + 1], "w");
                    if (stream) {
                        fprintf(stream, "%d %d", local_min, local_max);
                        fclose(stream);
                    }
                    close(pipefds[2*i + 1]);
                }
                
                free(array);
                if (pipefds) free(pipefds);
                if (filenames) {
                    for (int j = 0; j < pnum; j++) free(filenames[j]);
                    free(filenames);
                }
                free(child_pids);
                
                return 0;
            }
        } else {
            printf("Fork failed!\n");
            return 1;
        }
    }

    // Ожидаем завершения дочерних процессов с проверкой таймаута
    int completed_processes = 0;
    while (active_child_processes > 0 && !timeout_occurred) {
        int status;
        pid_t finished_pid = waitpid(-1, &status, WNOHANG);
        
        if (finished_pid > 0) {
            active_child_processes -= 1;
            completed_processes++;
        } else if (finished_pid == 0) {
            if (timeout_occurred) {
                printf("Timeout occurred! Killing child processes...\n");
                break;
            }
            usleep(100000); // проверяем таймаут каждые 100мс
        } else {
            if (errno != ECHILD) {
                perror("waitpid failed");
            }
            break;
        }
    }

    if (timeout_occurred) {
        printf("Terminating child processes due to timeout...\n");
        for (int i = 0; i < pnum; i++) {
            if (child_pids[i] > 0) {
                kill(child_pids[i], SIGKILL);
            }
        }
        
        // Ждем завершения убитых процессов
        while (active_child_processes > 0) {
            int status;
            pid_t finished_pid = wait(&status);
            if (finished_pid > 0) {
                active_child_processes -= 1;
            } else {
                break;
            }
        }
    }

    if (timeout_pid > 0) {
        kill(timeout_pid, SIGKILL);
        waitpid(timeout_pid, NULL, 0);
    }

    struct MinMax min_max;
    min_max.min = INT_MAX;
    min_max.max = INT_MIN;

    for (int i = 0; i < pnum; i++) {
        int min = INT_MAX;
        int max = INT_MIN;
        bool result_read = false;

        if (with_files) {
            FILE *file = fopen(filenames[i], "r");
            if (file) {
                if (fscanf(file, "%d %d", &min, &max) == 2) {
                    result_read = true;
                }
                fclose(file);
                remove(filenames[i]);
            }
        } else {
            close(pipefds[2*i + 1]);
            FILE *stream = fdopen(pipefds[2*i], "r");
            if (stream) {
                if (fscanf(stream, "%d %d", &min, &max) == 2) {
                    result_read = true;
                }
                fclose(stream);
            }
            close(pipefds[2*i]);
        }

        if (result_read) {
            if (min < min_max.min) min_max.min = min;
            if (max > min_max.max) min_max.max = max;
        }
    }

    struct timeval finish_time;
    gettimeofday(&finish_time, NULL);

    double elapsed_time = (finish_time.tv_sec - start_time.tv_sec) * 1000.0;
    elapsed_time += (finish_time.tv_usec - start_time.tv_usec) / 1000.0;

    free(array);
    if (pipefds) free(pipefds);
    if (filenames) {
        for (int i = 0; i < pnum; i++) free(filenames[i]);
        free(filenames);
    }
    free(child_pids);

    if (timeout_occurred) {
        printf("Execution terminated due to timeout after %d seconds\n", timeout);
        printf("Completed processes: %d/%d\n", completed_processes, pnum);
        if (min_max.min != INT_MAX && min_max.max != INT_MIN) {
            printf("Partial results from completed processes:\n");
            printf("Min: %d\n", min_max.min);
            printf("Max: %d\n", min_max.max);
        } else {
            printf("No processes completed in time\n");
        }
    } else {
        printf("Min: %d\n", min_max.min);
        printf("Max: %d\n", min_max.max);
    }
    
    printf("Elapsed time: %fms\n", elapsed_time);
    fflush(NULL);
    return timeout_occurred ? 1 : 0;
}